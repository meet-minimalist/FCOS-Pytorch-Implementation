...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        trainer = TrainingHelper(config_training)
        if args.ckpt_path:
            trainer.train(resume=True, resume_ckpt=args.ckpt_path)
        elif args.use_pretrain_ckpt:
            trainer.train(pretrained_ckpt=config_training.backbone_ckpt)
        else:
            trainer.train()
    ...
    if args.mode == 'overfit_train':
        ...
        if args.use_pretrain_ckpt:
            trainer.overfit_train(pretrained_ckpt=config_training.backbone_ckpt)
        else:
    ...
    if args.mode == 'infer':
        ...
        if not args.ckpt_path:
            raise ValueError('ckpt_path should be the exported ONNX/Torchscript/Pytorch pth model.')
        ...
        if os.path.splitext(args.ckpt_path)[1] == '.onnx':
            ...
            torchscript_model = True
        else:
            # Pytorch pth model to be loaded with Model definition from nn.Module class implmentation
            pass

        from InferModel import Inference

        infer_model = Inference()
        infer_model.run(args.img_path, args.ckpt_path, onnx_model, torchscript_model)